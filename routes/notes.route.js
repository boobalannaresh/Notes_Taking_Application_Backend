...
import { deleteMethod, getMethod, getMethodByID, postMethod, putMethodByID } from "../services/notes.service.js";
...
router.get("/", auth, async function (request, response) {
    ...
    const allDatas = await getMethod(queryData)
...
router.get("/:id", auth, async function (request, response) {
    ...
    singleData ? response.send(singleData) : response.status(404).send({ message: "Data not found" });
...
router.delete("/:id", auth, async function (request, response) {
    ...
    deleteDone.deletedCount > 0 ? response.send({ message: "Deleted Successfully Done" }) : response.status(404).send({ message: "Data Not Found" })
